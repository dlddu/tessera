...
- **유닛 집계 340건 (2026-08-13 실측, 측정 방법 고정)**: `test/unit/**`의 `^\s*(it|test)\(` 선언 수를 센다 — `grep -rhoE "^\s*(it|test)\(" test/unit | wc -l`. 이 방법은 트래커가 이전에 적은 값을 그대로 재현한다(PR #38 `8270b75` 시점 **269** · PR #39가 저작 시점 기준으로 적은 **305** = 269 + AC2.6 슬라이스 36). PR #39가 머지되기 전에 PR #41(`diagnostics.test.ts` **25**)·PR #42(`persistence-logging.test.ts` **10**)가 먼저 들어왔고 두 PR 모두 `docs/`를 건드리지 않아, 트래커의 305가 실측 340보다 **35건 뒤처져 있었다**(criterion ④). 두 신규 파일은 AC 버킷 회계에 들어가지 않는다 — 방어 대상이 AC가 아니라 아래 '플랫폼 인프라 표면'이기 때문이다.
- **플랫폼 인프라 표면은 값 사슬 밖 부록에 등재된다 (criterion ③ 해소)**: `src/main/diagnostics/`·`src/renderer/diagnostics/`의 사용자 대면 표면(`Debug` 메뉴 2항목·⌘⌥L·`TESSERA_DEBUG`·`TESSERA_LOG_LEVEL`·로그 파일과 회전·크래시 미니덤프·렌더러 console 릴레이)과 auto-update가 `platform/tessera-platform-infrastructure.md`에 등재되어 있다. **AC를 늘리지 않는다** — 두 표면 모두 V1~V4 중 어느 것도 달성하지 않으므로 가치 사슬에 넣으면 검증할 제품 시나리오가 없는 AC와 mockup 공백을 새로 만든다. 부록은 예외를 늘리는 장치가 아니라 예외로 두자던 표면을 SSOT 안에서 감사 가능하게 만드는 장치이며, 등재 자격·졸업 규칙이 그 문서에 함께 있다. 정합성 모델의 예외 조항(현재 auto-update 하나) 자체를 넓힐지는 **모델 소유자의 결정**이라 이 레포의 권한 밖이다. 진단 밖에 미등재 플랫폼 표면이 없음은 `src/` 전수 grep(`Menu.`·`accelerator:`·`shell.open`·`clipboard.`·`globalShortcut`·`openDevTools`)으로 확인했다 — 히트는 전부 `src/main/diagnostics/` 안이다.
...
> 경로는 `docs/` 기준이다. 배치를 정하는 구획 규칙과 고정점 규약은 [`README.md`](./README.md)에 있다.
> `(고정 경로)` 표시는 GitHub Pages 공개 URL과 `src/` 주석이 인용하는 앵커라 이동하지 않는다는 뜻이다.

| 종류 | 파일 |
|---|---|
| 문서 지도(구획 규칙·인덱스) | `README.md` |
| 가치 문서 | `values/tessera-values.md` |
...
| 사용자 여정 J2 (백엔드/컨테이너) | `journeys/tessera-journey-backend.md` |
| 사용자 여정 J3 (브라우저 라우팅/인증) | `journeys/tessera-journey-browser-routing.md` |
| 사용자 여정 J4 (상태 복원) | `journeys/tessera-journey-state-restoration.md` |
| 디자인 시스템 (문서) | `design-system/tessera-design-system.md` (고정 경로) |
| 디자인 시스템 (CSS) | `design-system/tessera.css` (고정 경로) |
| mockup (27개) | `mockups/M-Jx-Sn.html` (고정 경로) |
| mockup 갤러리 | `mockups/index.html` (고정 경로) |
| mockup 인덱스 | `mockups/tessera-mockup-index.md` (고정 경로) |
| 플랫폼 인프라 표면 (값 사슬 밖) | `platform/tessera-platform-infrastructure.md` |
...
| 2026-08-13 | **진단 표면 문서화 + 검증 축 재동기화**(정합성 criterion ③·④) — PR #41(파일 로그·크래시 훅·`Debug` 메뉴·DevTools escape hatch)·PR #42(레이아웃 골격 저장·재구성 로깅)가 `docs/`를 전혀 갱신하지 않아 생긴 두 어긋남을 닫는다. 값 사슬 밖 부록 `tessera-platform-infrastructure.md` 신설 — 등재 자격(비-가치·그릇을 위한 수단·코드 근거와 테스트 동반·표면 전량 기재)·졸업 규칙과 함께 진단(PI-1)·auto-update(PI-2)를 등재해 README에만 있던 예외 선언을 SSOT로 옮겼다. **AC를 늘리지 않는 최소 형태**를 고른 이유는 두 표면이 V1~V4 어느 것도 달성하지 않아 가치 사슬에 넣으면 검증할 시나리오 없는 AC와 mockup 공백이 새로 생기기 때문이다. 유닛 집계는 305 → **340**으로 재동기화하고 측정 방법(`test/unit/**`의 `^\s*(it|test)\(` 선언 수)을 문서에 고정했다(기존 값 269·305를 그대로 재현함을 커밋 대조로 확인). 가치·PRD·AC 본문·테스트 문서·여정·mockup·디자인 시스템 무변경(to-be 미하향), 코드·테스트 무변경(docs-only) | criterion ③ 미등재 표면 1건(진단 — `Debug` 메뉴·⌘⌥L·`TESSERA_DEBUG`/`TESSERA_LOG_LEVEL`·로그 파일·미니덤프·console 릴레이), criterion ④ 유닛 집계 305 대 실측 340(35건 뒤처짐), 문서 15종 | criterion ③ 미등재 표면 0건(진단·auto-update 부록 등재, `src/` 전수 grep으로 잔여 0 확인), criterion ④ 유닛 집계 340 = 실측(측정 방법 고정), 문서 16종. AC 26·집계 구현 19/부분 3/스텁 4·mockup 27·e2e spec 19 불변 |
| 2026-08-15 | **`docs/` 폴더 체계 정비**(문서 배치 구조 확정) — 최상위 평면 배치(md 16개)를 종류당 디렉터리 하나로 재편: `values/`·`prd/`·`tests/`·`journeys/`·`platform/` 신설(14개 `git mv`, 파일명 불변) + root에 문서 지도 `README.md` 신설(구획 규칙·문서 인덱스·링크 정책·고정점 규약). `mockups/`·`design-system/`은 Pages 공개 URL과 `src/` 주석 3곳의 앵커라 고정(옮기면 `src/` 편집 → release 경로 필터 → 실사용자 auto-updater 배포). 직전 갱신의 플랫폼 인프라 부록을 `platform/`으로 이동. 상대 링크 48건 정정 + `docs/` 전수 161개 링크 해석 확인(깨진 링크 0). AC·버킷·mockup·e2e·유닛 수치 전부 불변 | `docs/` 최상위에 사양 md 16개 평면 배치, 값 사슬 밖 표면의 자리가 규칙이 아니라 파일 하나의 위치로 결정됨 | 종류당 디렉터리 1개 + root는 진입점·메타만, 구획 규칙이 `docs/README.md`에 성문화, 깨진 링크 0 |
