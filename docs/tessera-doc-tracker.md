...
- **건강 상태**: ⚠️ **위험 있음** — 🔴 제품 소유자 미확정 1건 · ⚠️ **구현 gap**: 확정 26개 AC 중 구현 16 · 부분 4 · 스텁 6(정의 to-be 미달, 아래 '구현·검증 상태' 참조). 문서 사슬(가치→PRD→AC→테스트→목업)은 완전하나 실제 구현·자동화 검증은 부분이다.

> 백엔드 연결 구조(가치→PRD→AC→테스트)는 모두 이어져 있다(AC 26개 전부 가치·테스트 연결).
> 프론트엔드 사슬은 여정→mockup↔디자인 시스템까지 모두 이어졌다(시각화 27/27).
> **주의 — 위 완결은 '문서 사슬'의 완결이다.** 실제 구현(`src/`)·자동화 검증(`test/`)은 별개 축이며, 26개 AC 중 10개(부분 4·스텁 6)가 아직 미완이다(아래 '구현·검증 상태'). 남은 위험은 제품 소유자 미확정 1건 + 구현 gap이다.

## 구현·검증 상태 (as-is · 2026-07-21 실측)
...
> **구현 16 · 부분 4 · 스텁 6** (완전 충족 16/26). 근거는 `src/` 실측.

| 버킷 | 개수 | AC | 근거(요약) |
|------|------|-----|-----------|
| ✅ 구현 | 16 | AC1.1~1.7, AC2.1, AC2.4, AC2.5, AC2.7, AC2.8, AC3.1, AC3.2, AC3.5, AC4.4 | 레이아웃 전집·백엔드 선택/상속/parity·host 전용 영역·격리·방향 A 라우팅·브라우저 탭 URL 복원이 비게이트 자동화로 실검증 |
| ◐ 부분 | 4 | AC2.2, AC2.3, AC2.6, AC4.5 | AC2.2/2.3: 터미널·파일 I/O는 동작하나 `runProcess`/`getEnv`가 `NotImplementedError`(Host/ContainerBackend)이고 Claude 서피스는 정적 목업(`ClaudeSurface`). AC2.6: 컨테이너 start만, stop/remove(`backend.lifecycle`) `NotImplementedError`. AC4.5: host측 원자 저장은 동작하나 `buildWorkspaceSnapshot`이 `surfaces:[]`로 골격만 저장 |
| ✗ 스텁/미구현 | 6 | AC3.3, AC3.4, AC4.1, AC4.2, AC4.3, AC4.6 | AC3.3: `BrowserRouter.forwardCallback` throw(방향 B). AC3.4: 방향 B 부재로 auth 루프 미완결. AC4.1/4.2/4.3: `buildWorkspaceSnapshot`의 `surfaces:[]` 고정 → 에디터·Claude·터미널 내용 복원 없음. AC4.6: 복원↔재접속 충돌 처리 코드 전무(last-write-wins) |

- **값별 롤업**: V1(레이아웃) 7/7 ✅ · V2(백엔드) 5/8 ◐ · V3(브라우저 라우팅) 3/5 ◐ · V4(상태 복원) 1/6 ✗.
- **테스트 게이팅**: 컨테이너 실행 e2e(`M-J2-*`)는 `TESSERA_CONTAINER_E2E=1` 게이트로 기본 CI에서 `test.skip`. 스텁 AC(AC2.6-lifecycle·AC3.3·AC3.4·AC4.1·AC4.2·AC4.3·AC4.6)는 기능 부재로 자동화 테스트 없음.
- **다음 구현 우선순위(참고)**: PRD-4 복원 본체(AC4.1~4.3·4.6, `surfaces:[]` 실체화)와 PRD-3 방향 B(AC3.3·3.4)가 최대 gap, 이어 AC2.6 생명주기·AC2.2/2.3 Claude/runProcess. 순서·구현 여부는 후속 정합성 task의 계획 단계 몫.
- **범위 밖(이번 정합에서 미해소, gap 유지)**: `ClaudeSurface`의 존재하지 않는 `PRD-5 / J3` 참조, AC2.2/2.3 문구 vs Claude 목업 정합은 코드/문서 후속 판단 대상이다.
...
| V4: 작업 손실 없는 복원력 | PRD-4 (상태 복원) | AC4.1, AC4.2, AC4.3, AC4.4, AC4.5, AC4.6 | T-4 | ✅ 완전 | ✗ 1/6 (AC4.4만, 4.5 부분·4.1·4.2·4.3·4.6 스텁) |
...
- ⚠️ **단, 이는 문서 축(테스트 시나리오 존재)이며 자동화 검증과 다르다.** 구현 축에서 10개 AC(부분 4·스텁 6: AC2.2·2.3·2.6·3.3·3.4·4.1·4.2·4.3·4.5·4.6)는 기능 부재/부분으로 자동화로 실검증되지 않는다. 상세는 위 '구현·검증 상태' 섹션 참조.
