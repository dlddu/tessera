...
> **자동화 유닛 테스트 343건 / 25파일** (2026-09-20 `npm test` 실측, 전건 통과). 직전 기재값 305건은
> 2026-08-08 시점이며 그 뒤 세 슬라이스가 쌓은 증가분이 반영되지 않아 criterion ④ 불일치였다 —
> 이번 갱신으로 해소한다. 버킷별 AC 판정(19·3·4)은 그 사이 구현 축이 움직이지 않아 불변이다.
...
### 🟢 문서 없는 구현 (코드에만 있는 사용자 대면 표면)
- (없음) — **해소됨(2026-09-20)**. 가치 사슬(V→PRD→AC→T) 밖에 있으면서 사용자 눈에 보이던 두 표면,
  **진단**(`Debug` ▸ `Reveal Log Folder` ⌘⌥L · `Copy Log File Path` · `TESSERA_DEBUG` ·
  `TESSERA_LOG_LEVEL` · 로그 파일 회전 · 크래시 미니덤프)과 **auto-update**(재시작 어포던스)가
  `platform/tessera-platform-infrastructure.md`에 PI-1·PI-2로 등재됐다.
- **AC 회계 영향 없음** — 등재 항목은 가치를 달성하지 않으므로 AC가 아니며, 위 '구현·검증 상태'의
  26개 버킷에 들어가지 않는다. 등재는 면제가 아니라 표현이다(자동화 테스트 방어 의무는 그대로:
  PI-1 `diagnostics` 25건 + `persistence-logging` 13건, PI-2 `update` 6건).
- **전수 근거**(2026-09-20 재실측): `grep -rn "Menu\.\|new MenuItem\|accelerator:\|shell\.open\|clipboard\.\|globalShortcut\|openDevTools" src/`
  의 히트가 전부 `src/main/diagnostics/` 안이다(`attachWindowDiagnostics.ts` · `installDiagnosticsMenu.ts`).
  즉 등재되지 않은 앱 수준 메뉴·전역 단축키·클립보드·Finder 열기·DevTools 토글은 없다.
...
6. ✅ **문서 없는 구현 해소 — 플랫폼 인프라 표면 등재** — 완료(`platform/tessera-platform-infrastructure.md`,
   PI-1 진단 · PI-2 auto-update). 정합성 criterion ③.
7. ✅ **문서 폴더 체계 정비** — 완료(`values/`·`prd/`·`tests/`·`platform/` 구획 + 배치 규약 `README.md`
...
문서 목록의 정본은 **공개 허브 [`index.html`](./index.html)** 이다(`https://dlddu.github.io/tessera/`).
이 트래커가 같은 목록을 2벌째로 들고 있으면 한쪽만 갱신돼 곧바로 drift가 되므로, 여기서는 목록을 갖지
않는다 — 이 문서는 **상태**를, 허브는 **위치**를 맡는다. 배치 규칙은 [`README.md`](./README.md)에 있다.

문서가 늘거나 줄거나 옮겨지면 같은 커밋에서 허브를 고치고 `node scripts/check-docs-links.mjs`로
링크 해석을 확인한다.
