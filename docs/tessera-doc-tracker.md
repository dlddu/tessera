...
> 최근 갱신: 2026-07-05 · host=container 조작 동등성 + ⌘K 커맨드 팔레트(J2-S5/AC2.5) **구현 완료** — 키보드 커맨드를 공유 레지스트리(`src/renderer/commands/`)로 일급화: `Command{id,label,keycap,scope,run(ctx,e?),match(e)}` 스키마 + `dispatchKey`/`filterCommands`/`commandById`/`workspaceContext` 헬퍼. 기존 인라인 keydown 분기를 전량 이관 — `WorkspaceView`는 레이아웃 커맨드+⇧⌘W를, `App`은 ⌘N/⌘1–9를 레지스트리 경유로 디스패치(엣지케이스 보존: ⌘D 피커·Esc zoom 유예·`code==='Slash'`·Ctrl 배제, 기존 `M-J1-S*` e2e가 회귀 가드). 같은 레지스트리 위에 ⌘K 커맨드 팔레트(`CommandPalette`) 신설 — 레이아웃+워크스페이스 커맨드 **슈퍼셋**을 검색·실행하며 실제 키맵 키캡을 표시. 힌트 표면(`KeymapOverlay`·`StatusBar`)의 하드코딩 키캡을 레지스트리에서 파생(`Keycap` 컴포넌트)해 드리프트 차단. AC2.5는 조작·UI가 backend 무관이라 코드 격차가 없으므로 **증명**으로 검증 — 비게이트 단위(`command-parity`: backendKind만 다른 두 레이아웃에 동일 시퀀스 → 동일 `LayoutSnapshot` 구조·포커스; `command-registry`: match/dispatch/run/filter) + 게이트드 e2e `M-J2-S5`(⌘K 팔레트 UI 비게이트 + host=container 동등 시퀀스 게이트). M-J2-S5 목업 정정(팔레트 키캡 ⌃⌘→→⌥⌘→·⌃Tab→⇧⌘], 워크스페이스 커맨드 행 추가로 슈퍼셋 정합). 여정 J2 시각화 7/7·mockup 27개·AC 26개 불변, 상태 카운트 변화 없음. 남은 위험은 제품 소유자 미확정 1건.
