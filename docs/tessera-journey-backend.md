...
| 4 | 기본 영역에 pane/tab을 추가하면 모두 같은 컨테이너 환경(작업 디렉토리·env)을 상속 — 기본 영역에서는 backend 혼합 없음. | AC2.4 | ✅ [M-J2-S4](./mockups/M-J2-S4.html) |
...
| 7 | 필요 시 **host 전용 영역**을 열어 호스트 도구(호스트 셸·호스트 파일 편집 등)를 실행. 영역 경계가 시각적으로 구분되고, 컨테이너 기본 영역의 격리는 그대로 유지. | AC2.7, AC2.8 | ✅ [M-J2-S7](./mockups/M-J2-S7.html) |
