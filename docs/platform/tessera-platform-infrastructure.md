...
> 최근 갱신: 2026-09-20 · 폴더 체계 정비와 함께 `docs/platform/`에 신설 · 테스트 건수 재실측
> 이 문서는 **제품 가치(V1~V4) 사슬 밖**에 있는 사용자 대면 표면을 등재한다.
> 여기 등재된 항목은 AC·테스트 문서(T-N)·여정(J-N)·mockup을 갖지 않는다 — 가치를 달성하지 않기 때문이다.
>
> **왜 `platform/`에 있는가**: [`../README.md`](../README.md)의 구획 규칙 3이 "가치 사슬 밖 사용자 대면
> 표면은 `platform/`"으로 자리를 정한다. 이 문서는 그 디렉터리의 등재부이며, 자격·졸업 규칙을 아래에서
> 스스로 정의한다. 최상위에 부록 파일을 얹는 대신 구획을 만든 이유는, "어디에 두는가"가 파일 하나의
> 위치가 아니라 다음 표면에도 그대로 적용되는 **규칙**이어야 하기 때문이다.
...
- **자동화 테스트**: `test/unit/diagnostics.test.ts` 25건(레벨 게이팅·`parseLevel`·에러 직렬화·라인 포맷·
  회전 판정·console 레벨 매핑·소스 축약·⌘⌥L 코드 매칭), `test/unit/persistence-logging.test.ts` 13건
